...
### `POST /api/auto_trade`（SHADOW-only 自动交易开关，见 §10b）
```json
请求: { "enabled": bool }
返回: { "ok":true, "auto_trade":bool, "mode":"SHADOW|LIVE" }
```
...
## 10b. SHADOW-only 自动交易 + 按信号统计（对 §2 人在环铁律的有意收窄豁免）
...
- **开关**：右上角新拨钮 `AUTO`（`ST.auto_trade`）。`POST /api/auto_trade`。LIVE 下不可开（前端禁用 + 后端强制拒绝）。**落盘持久化**（`outputs/auto_trade_state.json`，`save_auto_trade_state`/`load_auto_trade_state`）——`ST.mode` 本身仍不落盘、重启回默认 `SHADOW`，但 `auto_trade` 会恢复成重启前的开关状态；用户明确要求：部署到服务器后，每次 `systemctl restart`（每次部署都会触发）不该悄悄把自主交易关掉却没人发现。
- **驱动方式（`_autonomous_trade_loop`，2026-07-24 起）**：`screen_once` 此前只由浏览器 `/api/run` 轮询触发——部署到服务器就是为了断电/关浏览器也能继续跑，但没人开着页面时后端其实完全静止，`AUTO` 开着也不会有任何新扫描/新交易/止盈止损检查。启动时（`@app.on_event("startup")`）无条件起一个后台线程，只要 `ST.mode=="SHADOW" and ST.auto_trade` 就按 `DEFAULT_POLL_S`(5.6s) 自己跑 `screen_once("sol")`（`TRADEABLE_CHAINS` 里唯一可交易链），`ST.lock` 保证不会跟浏览器发起的请求并发踩踏；单次异常被捕获+记日志(`AUTOLOOP_ERR`)，不会打断循环。`AUTO` 关着时线程只空转睡眠，不烧 CLI 配额。
- **入场**：`screen_once` 里，决策为 `action=="ACTION"`（已过全部闸门）且 `ST.auto_trade` 为真 → `auto_open_position` 自动开仓，**固定 `auto_size_usd`($20) 名义仓位**（换算成原生币数量：`$20 / native_usd_price(chain)`，兜底表 `NATIVE_USD_FALLBACK` 防 Mock 模式无原生币报价查询报错）。同地址已持仓（人工或自动）不重复开；`ST.auto_traded_addresses`（启动时从独立文件 `outputs/auto_traded_addresses.json` 读取，`save_auto_traded_addresses`/`load_auto_traded_addresses`）确保同一地址**永久**只自动入场一次，不是限时冷却——该文件独立于 `trade_decisions.jsonl`，统计重置（archive+truncate 日志）不会连带清空黑名单（真实事故：黑名单曾经只从日志 BUY 记录重建，用户清统计时顺带清空了黑名单，导致 BUNKEE 被同一循环买了两次）；`ath_mcap`(`history_highest_market_cap`) 与当前 `mcap` 的比例 `< min_auto_ath_ratio`(0.6) 也硬拦——`crowdedness=="early"` 只看最近 5m/1h 动能，不知道这个币今天是否已经拉升-砸盘过一轮（真实事故：BUNKEE 当前市值仅为历史最高市值的 29%，短期动能仍读作"early"）；`crowdedness=="crowded"`（LLM 判定 5m/1h 动能不同向、非清晰单边）直接拒绝自动入场——`priority_score` 本身不看 crowdedness，靠这里硬拦，避免追进已经涨跌过一轮、当前只是横盘/假反弹的死币；狙击钱包数 `sniper_count > max_auto_sniper_count`(10，2026-07-26 按 114 笔实盘数据定档：0 个 +9.5%／1-9 个 +19.4%／10-19 个 **-8.8%**／≥20 个 **-4.2%**，10 处断层)、流动性 `liquidity < min_auto_liquidity_usd`($3000) 同理硬拦——这两项此前只在人工界面当标签展示，从不影响自动交易的评分/决策；成交笔数 `swaps < min_auto_swaps`(50，buys+sells 之和) 或 1h 成交额 `vol_1h < min_auto_volume_usd`($10000) 也硬拦——`vol_1h` 此前从未被任何门槛用过，且 `buy_ratio` 这类比率信号在个位数笔数样本上是噪音（真实事故：9 分钟新币、图表几乎空白，仍给出 conviction 0.95）；`sm_confluence < min_auto_sm_confluence`(2) 或 `dev_eval < min_auto_dev_score`(0.30) 也硬拦——比共用流水线的最低门槛(`min_smart_money_confluence`=1、`min_dev_score`=0.15)更高，用户复盘发现连续几笔亏损的共同点不是"哪项不合格"，而是每项都刚好卡在及格线、毫无安全冗余（真实事故：三笔连续亏损 sm_confluence 全部=1，两笔 dev_score 正好=0.15）；机器人有独立子额度 `max_auto_positions=5`（不挤占人工 `max_concurrent_positions`）；仍过一遍 `RiskManager.gate()`；**年龄 `age_min > max_token_age_min`(15 分钟) 无条件拦截**（2026-07-26 按 114 笔实盘数据重设，原为 3 天 + conviction 例外）：年龄是所有维度里最干净、最单调的预测因子——<5min avgPnL +13.7%／5-15min +8.3%／15-60min **-5.5%**／≥60min **-19.8%（胜率仅 20%）**，超 15 分钟的单子累计 -280%，是最大的一处失血。原"信号极强则放行"例外已删除：conviction 实测不预测结果（0.9-0.95 桶 avgPnL -10.7%，反而最差），该例外实测只放行了 4 笔、合计 -94.8%，留着会把新门槛架空。**`dev.exited` 反过来是必要条件**（用户明确要求，2026-07-24 二次调整）：dev 必须已经清仓本币才买（不是"dev 已清仓则拒绝"，是"dev 未清仓则拒绝"）——理由：dev 若还握着仓位随时可能砸盘，dev 已出清则没有内部人能再靠抛售操纵价格；配合 `min_auto_dev_score`(0.30) 门槛把历史记录差的 dev 挡在外面（如 Vader 的 0.21），"dev 已出清 + 历史记录不差" 比 "dev 还在场" 更安全。`f.dev` 为 `None`（查不到历史）按未知处理，同样不买。曾经短暂反过来做过"dev 已清仓则拒绝"（同一个真实事故 Vader 触发），后经复盘认为方向搞反了才改成现在这样。
...
- **持仓新字段**（随 `positions.json` 免额外插件持久化）：`auto`、`entry_signal`(入场时的一次性快照，用于日后亏损复盘按维度分桶：verdict/conviction/crowdedness/dev_score/priority/sm_confluence/sniper_count/liquidity/vol_1h/swaps/buy_ratio/mcap/ath_mcap/ath_ratio/age_min/chg_5m/chg_1h/dev_exited——凡是自动入场时参与拦截或评分的原始值都存一份，否则平仓后无法判断"快速止损的单子当时各指标到底多少")、`orig_size_sol`(建仓时的原始数量，部分止盈后 `size_sol` 会缩小)、`tp1_done`、`peak_price`。前端持仓卡片对 `auto=true` 显示 `AUTO` 徽章 + 已部分止盈态。

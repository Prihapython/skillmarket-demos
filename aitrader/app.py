...
CFG = {
    ...
    # ⚠️ 这个数字**只适合验证机制，不适合真交易**。
    # 2026-07-27 按 139 笔实盘校准后的模型（见 WALLET_PLAN「перерахунок」一节）：
    #     $5 → -$33.72 | $10 → -$11.85 | $20 → +$31.91 | $50 → +$163.16 | $100 → +$381.93
    # 每笔约 $0.40 的固定成本（手续费 + token 账户租金）与仓位大小无关，
    # 占 $5 的 8%、占 $100 的 0.4%——所以**小仓位是结构性亏损**，
    # 「保守起见先小额」在这里恰恰是把策略做死的选择。真交易起步 $20，正常 $50 起。
    "live_size_usd": 5.0,
}
